...
class OrientationEvent {
  ...
  Matrix3 get rotationMatrix => quaternion.asRotationMatrix();

  /// The orientation of the device represented as euler angles (azimuth, pitch,
  /// roll).
  EulerAngles get eulerAngles {
    final mx = rotationMatrix;
    final x = asin(clampDouble(mx[5], -1, 1));
    final double y;
    final double z;
    if (mx[5].abs() < 0.9999999) {
      y = atan2(-mx[2], mx[8]);
      z = atan2(-mx[3], mx[4]);
    } else {
      y = 0;
  ...
  OrientationEvent remapCoordinateSystem(Axis3D newX, Axis3D newY) {
    ...
    final transformMatrix = Matrix3.columns(
      newX.vector,
      newY.vector,
      newZ.vector,
    );
    ...
    return OrientationEvent._(
      Quaternion.fromRotation(transformMatrix.transposed()) * quaternion,
      accuracy,
      timestamp,
      transformMatrix..multiply(coordinateSystem),
    );
  }
}
